...
void cadastrarProduto() {
    ...
    } else if (opcao == 2) { // Sabao de barra
        printf("Digite o tipo de peso:\n");
        printf("1. 70g\n");
        printf("2. 100g\n");
        printf("3. 150g\n");
...
void listarProdutos() {
    ...
    for (int i = 0; i < numProdutos; i++) {
        printf("descricao: %sPeso: %d\nValor de compra: R$ %.2f\nValor de venda: R$ %.2f\nValor do lucro: R$ %.2f\nPercentual de lucro: %.2f%%\nFabricante: %s\n\n",
               produtos[i].descricao, produtos[i].peso, produtos[i].valorCompra, produtos[i].valorVenda, produtos[i].valorLucro, produtos[i].percentualLucro, produtos[i].fabricante);
    }
    system ("pause");
    system("cls");
}
